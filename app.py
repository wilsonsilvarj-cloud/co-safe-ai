...
if not UFRJ_LOGO.exists():
    UFRJ_LOGO = IMAGENS_DIR / "Obsoleot" / "UFRJ LOGO.png"
...
def carregar_corpus_pdf(
    max_chars_total: int = 200000,
    max_chars_por_pdf: int = 20000,
):
    """
...
# ==================================================
# LEITURA DO ARQUIVO ENVIADO PELO USUÁRIO (PDF, WORD OU EXCEL)
# ==================================================
def extrair_texto_arquivo_upload(uploaded_file, max_chars: int = 15000) -> str:
    """
    Extrai texto do arquivo enviado pelo usuário (PDF, DOCX, XLSX/XLS).
    Não altera layout nem campos — apenas fornece mais contexto para a IA.
    """
    ...
    try:
        if nome.endswith(".pdf"):
            ...
            data = uploaded_file.getvalue()
            planilha = load_workbook(BytesIO(data), data_only=True)
            partes = []
            for aba in planilha.worksheets:
                partes.append(f"[Aba: {aba.title}]")
                for linha in aba.iter_rows(values_only=True):
                    valores = [str(v) for v in linha if v not in (None, "")]
                    if valores:
            ...
            for aba in xls.sheet_names:
                ...
                df = pd.read_excel(BytesIO(data), sheet_name=aba, engine="xlrd")
                for linha in df.fillna("").astype(str).values.tolist():
                    valores = [v for v in linha if v.strip()]
                    if valores:
                        partes.append(" | ".join(valores))
            texto = "\n".join(partes).strip()
        ...
        st.warning(f"Falha ao ler o arquivo enviado do acidente: {e}")
        return ""
...
def obter_cliente_openai():
    ...
    if not api_key:
        return None, (
            'OPENAI_API_KEY não configurada. No Streamlit Cloud, abra o app > Settings > Secrets '
            'e adicione OPENAI_API_KEY = "sua-chave".'
        )
    try:
...
ocupacionais no contexto brasileiro.
...
3) PRODUZIR TRÊS RESULTADOS, em JSON:
   (a) "relatorio_completo": um texto integrado, descrevendo o acidente e o encadeamento causal, com análise
       por nível HFACS, evitando culpabilização simplista.
...
- O campo "relatorio_completo" DEVE começar exatamente com a frase:
  ...
  (sem aspas, respeitando maiúsculas, acentuação e redação).
...
if submitted:
    ...
    # >>> NOVO: texto extraído do arquivo enviado pelo usuário (PDF, DOCX, XLSX/XLS)
    texto_arquivo_acidente = ""
    if arquivo_acidente is not None:
